...
- implementing the runtime capture
...
- [ ] The specs state that learner `stdout` and `stderr` are shown separately from mission feedback.
- [ ] The specs state that harmless extra output does not fail Mission 01 when the objective is still met.
- [ ] The specs define a dedicated `Program Output` panel and when it should be shown.
